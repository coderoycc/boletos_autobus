$('#btn-create-sale').on('click', async (e) => {
  ...
  const formPassenger = document.getElementById('passenger-data-form');
  ...
  if (seat == '') {
    ...
    toast(ACTION, 'Debe seleccionar un asiento', 'error', 2700);
    return;
  }
  const requestSale = await createSale([formClient, formSale, formPassenger]);
