...
$(document).ready(async function () {
  ...
  var sc = $('#seat-map-upper').seatCharts({
    ...
    click: function () {
      if (this.status() == 'available') {
        ...
        return 'available';
      } else if (this.status() == 'unavailable' || this.status() == 'reserved') {
  ...
  $('#button-update-seats').on('click', async function () {
    ...
    toast(ACTION, requestUpdateSeats.message, requestUpdateSeats.success ? 'success' : 'error');
  });
  /**
   * DATOS DEL VIAJE
   */
...
function seatsRecalculate() {
  const value = $('#precio-asiento').val() == '' ? 0 : $('#precio-asiento').val();
...
function generateInputHtml(seatNumber) {
  ...
  <div id="i_seat_${seatNumber}">
    ...
    <div class="input-group mb-3">
      ...
      <label class="input-group-text">Nro. ${seatNumber}</label>
      <input type="text" class="form-control" name="name_passenger[${seatNumber}]" >
...
async function showTooltip(seatNumber, id) {
  ...
  var fechaFormateada = ("0" + fecha.getDate()).slice(-2) + "/" + ("0" + (fecha.getMonth() + 1)).slice(-2) + "/" + fecha.getFullYear() + " " + ("0" + fecha.getHours()).slice(-2) + ":" + ("0" + fecha.getMinutes()).slice(-2);
